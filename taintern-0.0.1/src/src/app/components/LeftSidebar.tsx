...
const mainItems = [
  ...
  { to: '/centers', icon: Table2, label: 'Final Centers', sub: 'Bảng Lương' },
  { to: '/master-ae', icon: Users, label: 'Master AE', sub: 'Final AE' },
  { to: '/audit', icon: ShieldCheck, label: 'Audit', sub: 'Đối soát' },
  { to: '/payment', icon: CreditCard, label: 'Bulk Payment', sub: 'Thanh toán' },
  { to: '/pivot', icon: Table2, label: 'Pivot Sheet 1', sub: 'Báo cáo' },
];
...
export function LeftSidebar({ isCollapsed, onToggle, onOpenSettings }: SidebarProps) {
  ...
  const renderNavItems = (items: typeof mainItems, label: string) => (
    <>
      {!isCollapsed && (
  ...
  return (
    <motion.div
      ...
      className="relative h-full shrink-0 flex flex-col z-50 bg-white/80 backdrop-blur-xl border-r-2 border-border/10"
    >
      {/* Logo Section */}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-4">
        {renderNavItems(mainItems, 'Main Modules')}
